...
@NamedNativeQueries({ 
	@NamedNativeQuery(name = "StrategyStockQuote.getTopXByStockNum",  resultClass=StrategyStockQuote.class,
		query = "select ssq.* from strategy_stock_quotes ssq join stock_quotes sq on (ssq.stock_quote_id= sq.stock_quote_id) where ssq.strategy_id = :strategyId and sq.stocknum = :stockNum order by sq.stock_quote_id desc limit 13") ,
	@NamedNativeQuery(name = "StrategyStockQuote.getAllByStockNumStrategyId",  resultClass=StrategyStockQuote.class,
		query = "select ssq.* from strategy_stock_quotes ssq join stock_quotes sq on (ssq.stock_quote_id= sq.stock_quote_id) where ssq.strategy_id = :strategyId and sq.stocknum = :stockNum order by sq.stock_quote_id desc limit 75") })
